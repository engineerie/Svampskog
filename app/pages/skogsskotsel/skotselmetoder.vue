...
>
  ...
  >
    <NuxtImg
      ...
      format="webp"
      quality="80"
      class="hidden md:block border border-neutral-200/60 rounded-lg transition-transform duration-300 ease-in-out
             "
    />
     <NuxtImg
      :src="`/images/${method.image}`"
      ...
      format="webp"
      quality="80"
      class=" md:hidden border border-neutral-200/60 rounded-lg transition-transform duration-300 ease-in-out
             "
    />
    <div
    v-if="!selectedMethod"
      class="absolute inset-0 rounded-lg
             ...
             pointer-events-none z-10 group-hover:opacity-50 transition-all"
    ></div>
    <div
    v-if="!selectedMethod"
...
icon="i-heroicons-x-mark"
  ...
  <UModal fullscreen title="Skogsskötsel" v-if="showModel">
    <UButton size="xl" label="Helskärm med jämförelsefunktion" color="neutral" variant="subtle" class="mt-4 ml-2" />
 ...
 <Transition name="fade">
   <div v-if="showModel" ref="modelSRef">
     <ModelS
       :current-framework="selectedFrameworksProp[0]"
       class="w-full mb-4"
     />
   </div>
 </Transition>
