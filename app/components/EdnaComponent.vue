            ...
            <div class="flex w-full justify-between gap-3">
              <div>
                ...
                <div class="flex gap-2">
                  <h1 class="text-md text-neutral-500">{{ speciesCountLabel }} arter baserat på {{ sampleEnvCount }}
                    skogar
                  </h1>
              ...
              <div class="absolute -top-2 right-0 flex flex-col justify-end">
                <UTabs v-if="showBarChart && activeTab === 'columnChart' && !isSmallScreen" v-model="chartTypeTab"
                  :items="chartTypeTabs" size="md" :ui="{
                    root: '',
                ...
                <div class="w-full flex justify-end">
                  <USelect v-model="treemapGroupingMode" :items="treemapGroupingOptions" item-value="value"
                    item-label="label" class="ring-muted/50 w-fit" :ui="{ content: 'min-w-fit' }"
                    v-if="showBarChart && activeTab === 'columnChart' && chartTypeTab === 'treemap' && !isSmallScreen">
                    <template #default="{ ui }">
                      <span v-if="selectedTreemapGroupingItem"
                        :class="ui.value({ class: 'flex items-center gap-2 min-w-0 overflow-hidden' })">
                        <UIcon :name="selectedTreemapGroupingItem.icon" class="size-4 text-neutral-500 shrink-0" />
                        <span class="truncate">{{ selectedTreemapGroupingItem.label }}</span>
                      </span>
                      <span v-else :class="ui.placeholder({ class: '' })">Välj gruppering</span>
                    </template>
                    <template #item="{ item }">
                      <div class="flex items-center gap-2">
                        <UIcon v-if="item.icon" :name="item.icon" class="size-4 text-neutral-500" />
    ...
    <transition name="fold-down">
      <UContainer
        ...
        <div class="w-full mx-auto max-w-7xl py-2 px-4">
          <BarChart :key="`sticky-${barChartInstanceKey}`" :chartData="data" :chartWidth="chartWidth" :chartHeight="40"
            :showControls="false" :showYAxis="false" :showTooltip="false" :geography="geographyValue"
            :forestType="forestTypeValue" :standAge="standAgeValue" :vegetationType="vegetationTypeValue"
            :matsvampFilter="matsvampFilter" :giftsvampFilter="giftsvampFilter" :gruppFilter="gruppFilter"
            :statusFilter="statusFilter" :search-term="effectiveSearchTerm" :sort-order="tableSorting"
            :visible-range="tableVisibleRange" :view-mode="chartView" :show-mode-tabs="false"
            @update:viewMode="chartView = $event" />
        </div>
      </UContainer>
    </transition>
